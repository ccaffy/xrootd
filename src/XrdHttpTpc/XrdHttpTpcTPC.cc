...
bool TPCHandler::mismatchReprDigest(const std::map<std::string, std::string> & passiveSrvReprDigest, XrdHttpExtReq &req,
    ...
    if(passiveSrvReprDigest.size()) {
        for (const auto & [digestName, digestValue]: passiveSrvReprDigest) {
            auto clientDigestMatch = XrdOucTUtils::caseInsensitiveFind(req.mReprDigest, digestName);
            if (clientDigestMatch != req.mReprDigest.end()) {
                ...
                if (clientDigestMatch->second != digestValue) {
                    ...
                    errMsg << "The client-provided checksum"
                           << " (" << clientDigestMatch->first << "=" <<  clientDigestMatch->second << ")"
                           << " mismatches the one coming from the remote server"
                           << " (" << digestName << "=" << digestValue << ")";
...
//  curl_easy_setopt(curl,CURLOPT_SOCKOPTFUNCTION,sockopt_setcloexec_callback);
    ...
    uint64_t sourceFileContentLength = 0;
    {
        ...
        if(!success || mismatchDigests) {
          ...
          // does not match the source file one, we fail here and don't open the file
          return 0;
        }
    }
